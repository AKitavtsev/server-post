  ...
  where
    ...
    deleteByID pool model id_ = do
        _ <- case model of
            ...
            "comment" -> liftIO $ execSqlT pool [id_] "DELETE FROM comment WHERE id_=?"
            "tag_draft" -> liftIO $ execSqlT pool [id_] "DELETE FROM  tag_draft WHERE draft_id=?"
            "photo_draft" -> liftIO $ execSqlT pool [id_] "DELETE FROM  photo_draft WHERE draft_id=?"
            _ -> return (0 :: Int64)
        return ()
        ...
        _ <-
            liftIO $
                execSqlT
                    pool
                    [id_, auth]
                    "DELETE FROM draft WHERE draft_id=? AND user_id =?"
        return ()
    ...
    updateDraft pool draft auth = do
      ...
      where
        ...
        pass _ = Nothing
        newValue a na = if isNothing a then Nothing else Just na
        ...
        _ <-
            liftIO $
                execSqlT
                    pool
                    [draft, auth]
                    "DELETE FROM post WHERE draft_id=? AND user_id =?"
        res <-
            liftIO $
                fetch
                    pool
        ...
        _ <-
            liftIO $
                execSqlT
                    pool
                    [id_, auth]
                    "DELETE FROM comments WHERE id_=? AND user_id =?"
        return ()
