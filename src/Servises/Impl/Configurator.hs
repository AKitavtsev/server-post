    ...
    getconfig fm = do
      ...
      case fm of
        SC.DB -> do
          ...
          password <-
            C.lookupDefault "" conf (T.pack "database.password") :: IO String
          limit <-
            C.lookupDefault 1 conf (T.pack "database.limit") :: IO Integer
          return (SC.DbConfig name user password limit)
        SC.LOG -> do
          levelStr <-
            C.lookupDefault "INFO" conf (T.pack "logger.level") :: IO String
          let level =
                case levelStr of
                  "DEBUG" -> DEBUG
                  "WARN" -> WARN
        ...
        SC.TOKEN -> do
          lifetime <-
            C.lookupDefault 86400 conf (T.pack "token.lifetime") :: IO Integer
